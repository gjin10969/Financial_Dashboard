...
async def all_metrics_json_data(start_date, end_date):
    ...
    try:
        ...
        with open('account_metrics.json', 'r') as file:
            data = json.load(file)
...
async def all_metrics_json_data_entry_data_only(start_date):
    if not await validate_dates(start_date, end_date):
        return None
...
async def data_computations(cached_data, start_date, end_date, mirror_account, calculation_types, symbols):
    ...
    try:
        ...
        filtered_data_list = await metrics_json_data(start_date, end_date, mirror_account)
        ...
        async def run_calculation(calculation_type):
            if calculation_type in calculation_functions:
                if calculation_type == 'max_draw':
                    return calculation_type, await calculate_max_draw(filtered_data_list,filtered_df, mirror_account, start_date, end_date)
        ...
        computation_results = await computation2(filtered_data_list, api_data, mirror_account, start_date, end_date)
...
async def value():
    ...
    account_metrics2 = await all_metrics_json_data_entry_data_only(start_date)
    ...
    overall_account = {
        ...
        'all_metrics_json_data_entry_data_only': account_metrics2,
    ...
    print('\n\n\n\n')
    print(account_metrics2.get('total_balance_data'))
