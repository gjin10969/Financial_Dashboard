...
async def wallet_balance(client):
    try:
        wallet_account = await asyncio.to_thread(client.futures_account, recWindow=1000)
...
async def get_client(account):
    async def thread_get_client(account):
    ...
    return await asyncio.to_thread(thread_get_client, account)
...
async def account_metrics_for_client(account_name, client_instance):
    initial_balance_for_account = initial_balance[account_name]
    total_balance_for_account = await wallet_balance(client_instance)
    perc_return_for_account = percent_return(total_balance_for_account, initial_balance_for_account)
    unrealized_pnl_for_account = float((await asyncio.to_thread(client_instance.futures_account))['totalUnrealizedProfit'])
    total_balance_for_account = float((await asyncio.to_thread(client_instance.futures_account))['totalWalletBalance'])
...
async def account_metrics(account_name=None):
    total_return = 0.0
    ...
    if account_name:
        ...
        if client:
            account_metrics_result = await account_metrics_for_client(account_name, client)
            return account_metrics_result

    else:
        tasks = [account_metrics_for_client(account, client) for account, client in clients.items()]
...
async def data_computations(cached_data, start_date, end_date, mirror_account, calculation_types):
...
async def calculate_total_fees(filtered_df, start_date, end_date):
    ...
    try:
        ...
        start_ts = int(datetime.strptime(start_date.strftime('%Y-%m-%d'), '%Y-%m-%d').timestamp() * 1000)
        end_ts = int((datetime.strptime(end_date.strftime('%Y-%m-%d'), '%Y-%m-%d') + pd.Timedelta(days=1)).timestamp() * 1000)
        klines = await asyncio.to_thread(client.get_historical_klines, 'BNBUSDT', Client.KLINE_INTERVAL_4HOUR, start_ts, end_ts)

        price_data = {}
        for kline in klines:
            open_time = datetime.fromtimestamp(kline[0] / 1000).date()
...
calculation_types = ['total_realized_pnl', 'calculate_total_fees', 'calculate_trading_days', 'winning_days', 'total_winning_trades', 'max_draw','total_loss_realized_pnl', 'total_net_profit_loss', 'losing_days','breakeven_days', 'average_profit', 'average_loss', 'profit_loss_ratio']
...
async def computation2(api_data, account_name, start_date):
    ...
    # Account selection
    metrics = await account_metrics(account_name)
    ...
    # Count trades and sum wins
    BTC_trades = df_BTC['realizedPnl'].count()
    BTC_win = df_BTC['win/lose'].sum()

    # Check if there are no trades (BTC_trades == 0)
    if BTC_trades == 0:
        BTC_win = 0
        BTC_winrate = 0
    else:
    ...
    #FILTER BY UNI
    df_UNI = df[df['symbol'] == 'LTCUSDT'].fillna(0)
    ...
    return {"overall": overall, "trades": trades, "winrate_per_symbol": winrates, "mirror_winrates": mirror_winrates}
...
#     result = await asyncio.create_task(data_computations(cached_data, start_date, end_date, mirror_account, calculation_types))
#     # print(cached_data)
