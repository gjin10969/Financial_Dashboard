...
# Function to calculate the sum of initial balances for all accounts
# def sum_of_initial_balance():
#     total_initial_balance = sum(initial_balance.values())
#     print(f"Total Initial Balance: {total_initial_balance}")
#     return total_initial_balance
...
async def validate_dates(start_date, end_date):
    try:
        ...
        start_date_utc = start_date_dt.replace(tzinfo=pytz.UTC)
        end_date_utc = end_date_dt.replace(tzinfo=pytz.UTC)
...
async def data_computations(cached_data, start_date, end_date, mirror_account, calculation_types):
    ...
    try:
        # Filter data based on the mirror account if provided
        if mirror_account:
        ...
        # Parse the start and end dates as UTC datetime objects
        start_date_dt = datetime.strptime(start_date, "%Y-%m-%d").replace(tzinfo=pytz.UTC)
        end_date_dt = datetime.strptime(end_date, "%Y-%m-%d").replace(tzinfo=pytz.UTC)

        # Filter the dataframe based on the UTC dates
        filtered_df = df[(df['date'] >= start_date_dt) & (df['date'] <= end_date_dt)]

        results = {}

        # Iterate over each calculation type and use the corresponding function to compute the result
...
end_date = input("PLEASE INPUT END_DATE (e.g., 2024-03-25): ") or "2024-07-01"
mirror_account = input("PLEASE INPUT account: ") or 'your_account'
...
#     task = asyncio.create_task(data_computations(cached_data, start_date, end_date, mirror_account, calculation_types))
#     result = await task
...
#     #total trades
#     trades_account = result2.get("trades")
#     symbols_per_account_data = result2.get("winrate_per_symbol")
...
#     winning_rate_per_account_dict = result2.get('mirror_winrates')
...
#     trades_account = {k: int(v) for k, v in result2.get("trades").items()}
