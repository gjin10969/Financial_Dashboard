...
def sql_live_data_scheduler():
    script_path = r"C:\Users\User\Documents\jonathan-dashboard\PINAKA_LATEST\financialDashboard\dashboard\sql_live_data_3.py"
    # print(f"Running script: {script_path}")
...
def binance_data_api_scheduler():
    script_path = r"C:\Users\User\Documents\jonathan-dashboard\PINAKA_LATEST\financialDashboard\dashboard\binance_data.py"
    # print(f"Running script: {script_path}")
    result = subprocess.run(['python', script_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    if result.returncode == 0:
        print("Script ran successfully.")
