...
if __name__ == "__main__":
    ...
    pth = os.path.join(output_dir, f"{symbol}_svm_backtest1.csv")
    acc_df.to_csv(pth)
    ...
    acc_report.update({"accuracy": {"precision": '--',
                                    ...
                                    "support": acc_report['macro avg']['support']}})
    acc_df = pd.DataFrame(acc_report).transpose()
    pth = os.path.join(output_dir, f"{symbol}_svm_backtest.csv")
    acc_df.to_csv(pth)
    # mlflow.log_artifact(pth)
    ...
    if len(trades) > 20:
        ...
        trades.to_csv(output_dir + f"/tradesheet_{symbol}.csv")
        # mlflow.log_artifact(output_dir + "/tradesheet_{}.csv".format(args.run_name))

        report = analyze_tradesheet(trades, output_dir)

        df_styled = report.style.background_gradient()
        dfi.export(df_styled, output_dir + "/reports.png", table_conversion = "matplotlib")
        # mlflow.log_artifact(output_dir + "/reports.png")
